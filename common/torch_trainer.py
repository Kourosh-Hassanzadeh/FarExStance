...
class TorchTrainer:
    def __init__(self, model, train_dataloader, valid_dataloader, optimizer, loss_fn,
             device, accumulation_steps, log_func,
             scheduler=None, vectorizer=None, test_dataloader=None):
    ...
    def test_model(self, model_save_path, visualize_attention = False
      ...
      self.log(("-" * 30) + "\n")        
      with torch.serialization.safe_globals([TransformerWithExtraFeatures]):
        self.model = torch.load(model_save_path, weights_only=False).to(self.device)
      self.model.eval()
